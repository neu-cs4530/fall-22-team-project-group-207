import { addVectors, angleBetween, crossProduct, magnitude, unitVector, scale, subtractVectors, vector } from "../Vector";

const BALL_CLOTH_ANGULAR_DECELERATION_RATE: number = 10; //rad/s^2
const BALL_CLOTH_ROLLING_FRICTION: number = 0.01;
const BALL_CLOTH_SLIDING_FRICTION: number = 0.2; // tentative
const GRAVITATIONAL_CONSTANT = 9.8; // m/s^2
const BALL_RADIUS = 0.028575; // m
const ANGULAR_SLIDING_DECEL_COEFF = 5 * BALL_CLOTH_SLIDING_FRICTION * GRAVITATIONAL_CONSTANT / (2 * BALL_RADIUS);
...
export default class PoolBall {
    ...
    public tick(elapsedTime: number) {
        if (this._isMoving) {
            ...
            if (!this._isAirborne) {
                ...
                if (this.angularVelocity.x !== this.velocity.x / BALL_RADIUS || this.angularVelocity.y !== this.velocity.y / BALL_RADIUS) {
                    // sliding
        ...
        if (this.angularVelocity.z !== 0) { this._updateSpinningBall(elapsedTime); }
    }

    private _updateOrientation(elapsedTime: number) {
        this._angularOrientation = addVectors(this.angularOrientation, scale(this.angularVelocity, elapsedTime));
    }
    ...
    private _updateSpinningBall(elapsedTime: number) {
        ...
        if (changeInSpin > Math.abs(this.angularVelocity.z)) {
            ...
            this._angularVelocity.z = this.angularVelocity.z > 0 ? this.angularVelocity.z - changeInSpin : this.angularVelocity.z + changeInSpin;
        }
    }

    private _updateRollingBall(elapsedTime: number) {
        const oldVelocityMagnitude = magnitude(this.velocity);
        const frictionVector: vector = scale(unitVector(this.velocity), BALL_CLOTH_ROLLING_FRICTION * GRAVITATIONAL_CONSTANT * elapsedTime);
    ...
    private _updateSlidingBall(elapsedTime: number) {
        ...
        const relativeVelocity: vector = addVectors(this.velocity, crossProduct(scale(kHat, BALL_RADIUS), this.angularVelocity));
        const totalFrictionDirection: vector = unitVector(relativeVelocity);
        this._velocity = subtractVectors(this.velocity, scale(totalFrictionDirection, BALL_CLOTH_SLIDING_FRICTION * GRAVITATIONAL_CONSTANT * elapsedTime));
